...
while True:
    ...
    for i in range(3):
        ...
        temp.sort(key=lambda x: x[1])  # if the game has not ended, sort the game based on rtt's
        ...
        cumu.sort(key=lambda x: x[1], reverse=True)  # sorting cumulative score array
        sleep(0.1)
        for a in clients:
